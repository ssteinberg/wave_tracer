...
/**
 * @brief Simple point perspective sensor. Not a physical sensor.
 * @tparam polarimetric use a polarimetric underlying film
 */
...
protected:
    /**
    * @brief Computes the sensor position on the near plane.
    *        Returns un-normalized results (i.e. z==1)
    * @param film_pos film (pixel) position
    */
    ...
    /**
    * @brief Computes the film position given a local direction from the sensor
    * @param dir direction
    */
    ...
    /**
     * @brief Partial derivative of camera direction w.r.t. x film coordinate.
     */
    [[nodiscard]] inline auto direction_pdvx() const noexcept { return ddir_dx; }
    /**
     * @brief Partial derivative of camera direction w.r.t. y film coordinate.
     */
...
public:
    ...
    /**
     * @brief Returns the sensor physical extent.
     */
    [[nodiscard]] inline const auto& sensor_extent() const noexcept { return snsr_extent; }

    /**
     * @brief Returns a sensor element's physical extent.
     */
    ...
    /**
     * @brief Spectral sensitivity is ignored here, the sensor response function is applied when splatting to film.
     */
...
/**
 * @brief Simple point perspective sensor.
 */
using perspective_scalar_t = perspective_t<false>;
/**
 * @brief Simple point perspective sensor.
 *        Polarimetric version: film consists of 4 targets quantifying the incident Stokes parameters vector.
 */
