...
/**
 * @brief General purpose 1/2/3-dimensional array of sensor elements.
 *
 * @tparam Dims film dimensions
 * @tparam polarimetric indicates whether film is polarimetric: polarimetric films keep track of the polarization state of the input, quantified via Stokes parameters vectors.
 */
...
private:
    /**
     * @brief The sensor's response given a sample
     */
...
public:
    /**
     * @brief Construct a new film object. 'Reconstruction' means splatting the (constant) Gaussian footprint of a sourced beam.
     * 
     * @param rfilter_stddev standard deviation, in film elements, of Gaussian footprint for beam reconstruction
     * @param block_size     size of a block, in film elements
     */
    ...
    /**
     * @brief Creates the sensor storage, that is used as a render target for rendering.
     */
    ...
    /**
     * @brief Acquires a film block for rendering. Not thread safe. Lazily caches blocks to avoid unneeded allocations.
     * 
     * @return block_handle_t
     */
    ...
    /**
     * @brief Splats an integrator sample onto the film storage from a thread pool worker. Used for direct sensor sampling techniques. ``storage`` must have been created with ``sensor_write_flags_e::writes_direct_splats`` flag.
     * (thread safe)
    ...
    /**
     * @brief Splats an integrator sample onto a film block. ``storage`` must have been created with ``sensor_write_flags_e::writes_block_splats`` flag.
     * (block expected to be accessed from this thread only)
