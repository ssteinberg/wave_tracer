...
/**
 * @brief A map of user-supplied defines.
 */
...
struct progress_callback_t {
    ...
    /** @brief Called on loading error and termination.
     */
...
/**
 * @brief Handles queueing and synchronizing the loading of scene element.
 *        Generic class: overloads are expected to provide the data source.
 */
class loader_t {
public:
...
public:
    ...
    /**
     * @brief Block and wait for loading of all shapes to complete.
     */
    void wait_shapes() const;
    /**
     * @brief Block and retrieve list of all shapes.
     */
    ...
    /**
     * @brief Returns TRUE if errors occurred during loading.
     */
    [[nodiscard]] inline bool has_errors() const noexcept { return success.load()==false; }

    /**
     * @brief Blocks and wait for loader to complete, then constructs the scene. 
     *        Only thread safe with sensors', sampler's and shapes' waiters and getter.
     *        NOT thread safe with wait().
     */
    std::unique_ptr<scene_t> get();

    /**
     * @brief Blocks and wait for loader to complete.
     *        Only thread safe with sensors', sampler's and shapes' waiters and getter.
     *        NOT thread safe with get().
     */
    void wait() const;

    /**
     * @brief Get the node with specified id. Nested nodes are ignored.
     */
    ...
    /**
     * @brief Enqueues an arbitrary auxiliary task as part of scene loading.
     *        Scene loading will wait for these auxiliary tasks to complete.
     *        These tasks are not scheduled on the threadpool to wait deadlocks on waits.
     */
    ...
    /**
     * @brief Notifies the loader that ``resource`` depends on ``dependency``.
     *        When complete_loading_tasks_for_resource() is called on ``resource``, it will also block and wait for any tasks queued by ``dependency``.
     */
    ...
    /**
     * @brief Waits and completes all auxiliary deferred loading tasks for a particular resource are done.
     *        Blocking operation.
     */
...
protected:
    /**
     * @brief Enqueues the loading tasks.
     *        Subclasses should call this when ready, passing the root node of the scene data. ``scene_node`` must remain valid for the lifetime of the loader.
     */
    void load(node_t* scene_node,
              const defaults_defines_t& user_defines);
    /**
     * @brief Marks that loading has failed.
     */
