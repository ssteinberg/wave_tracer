...
/**
 * @brief Phase-space extent of a beam. 
 */
struct phase_space_extent_t {
    ...
    /**
    * @brief The space-bandwidth product (SBP) of a beam with this phase-space extent
    */
    ...
    /**
     * @brief Return TRUE if this phase-space extent is physical, i.e. conforms to the uncertainty relation (SBP >= ¼).
     */
    ...
    /**
     * @brief Enlarges the phase-space extent by a scale factor.
     */
    ...
    /**
     * @brief Enlarges the phase-space extent to make it physical, if needed.
     *        Does nothing if the extent already satisfies minimum uncertainty, or is degenerate.
     */
    ...
    /**
     * @brief Computes a minimum-uncertainty beam's (MUB) tan(ɑ) given a spatial extent.
     *        For degenerate cases (vanishing spatial extent), returns a ray (tan(ɑ)=0).
     * @param spatial_length spatial extent length per dimension
     */
     ...
     * @brief Computes a minimum-uncertainty beam's (MUB) tan(ɑ) given a spatial extent.
     *        For degenerate cases (vanishing spatial extent), returns a ray (tan(ɑ)=0).
     * @param spatial_extent spatial extent (area)
     */
    [[nodiscard]] static constexpr inline f_t minimum_uncertainty_tan_alpha(
     ...
     * @brief Computes a minimum-uncertainty beam's (MUB) tan(ɑ) given a spatial extent.
     *        For degenerate cases (vanishing spatial extent), returns a ray (tan(ɑ)=0).
     * @param spatial_lengths spatial extent lengths for each dimension
     */
    [[nodiscard]] static constexpr inline f_t minimum_uncertainty_tan_alpha(
    ...
    /**
     * @brief Computes a minimum-uncertainty beam's (MUB) spatial extent given a tan(ɑ).
     *        For degenerate cases (vanishing tan(ɑ)), returns a point (spatial extent = 0).
     */
...
struct sourcing_geometry_t {
    ...
    /**
     * @brief Computes the phase-space extent of this sourcing geometry.
     */
    ...
    /**
     * @brief Computes the beam envelope (elliptic cone) for this sourcing geometry.
     */
    ...
    /**
     * @brief Source an isotropic minimum-uncertainty beam (MUB) into a solid angle.
     */
    ...
    /**
     * @brief Source an isotropic minimum-uncertainty beam (MUB) from a spatial footprint.
     */
    ...
    /**
     * @brief Source a minimum-uncertainty beam (MUB) from a spatial footprint.
     * @param x tangent direction for the largest spatial dimension
     */
    ...
    /**
     * @brief Source a minimum-uncertainty beam (MUB) from a surface.
     */
    ...
    /**
     * @brief Source a beam with explicitly defined spatial footprint and tan opening half angle.
     *        This can be used to specify a beam that is not strictly physical (not fulfilling the uncertainty relation); this is useful for not-strictly-physical sensors, like virtual cameras.
     */
    ...
    /**
     * @brief Source an isotropic beam with an explicitly defined phase-space extent.
     *        This can be used to specify a beam that is not strictly physical (not fulfilling the uncertainty relation); this is useful for not-strictly-physical sensors, like virtual cameras.
     */
