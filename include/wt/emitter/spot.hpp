...
/**
 * @brief A simple spot emitter. The spot emitter radiates from a singular point origin into a fixed solid angle.
 *        The `falloff' angle specifies the angle from the mean direction where the radiant intensity starts to linearly decrease, reaching 0 at `cutoff' degrees.
 */
...
public:
    ...
    /**
     * @brief Returns power fraction in beam as function of direction
     * @param direction direction in local coordinates (beam mean direction aligns with z-axis)
     * @return 
     */
    ...
    [[nodiscard]] inline beam::sourcing_geometry_t sourcing_geometry(const wavenumber_t k) const noexcept {
        // spot sources are not physical, default to a fake a spatial extent of 5λ
