...
// from: https://gist.github.com/ddovod/be210315f285becc6b0e455b775286e1
...
constexpr unsigned int colors256[] = {
		0xff000000,
		0xff000080,
		0xff008000,
		0xff008080,
	...
	inline bool ParseColor(const char* s, ImU32* col, int* skipChars)
	{
		if (s[0] != '\033' || s[1] != '[') {
			return false;
		}
		...
		std::string colorStr;
		[[maybe_unused]] bool bright = false;
		int bitNumber;
		bool readColor = false;
		for (const auto& el : _detail::split(seq, ";")) {
			if (colorStr.empty()) {
				if (el[0] == '3' && el.size() == 2) {
					colorStr = el;
				}
		...
		// all colours are set to "bright"
		if (!colorStr.empty()) {
			switch (colorStr[1]) {
				case '0': *col = 0xffaaaaaa; break;
				case '1': *col = 0xffaaaaff; break;
				case '2': *col = 0xffaaffaa; break;
				case '3': *col = 0xffaaffff; break;
				case '4': *col = 0xffffaaaa; break;
				case '5': *col = 0xffffaaff; break;
				case '6': *col = 0xffffffaa; break;
				case '7': *col = 0xffffffff; break;
				case '8': *col = _detail::colors256[bitNumber]; break;
	...
	inline void ImFont_RenderAnsiText(
            ImFont* font,
			...
			bool cpu_fine_clip = false)
	{
		...
		while (s < text_end) {
			...
			if (word_wrap_enabled) {
				...
				if (!word_wrap_eol) {
					word_wrap_eol = font->CalcWordWrapPosition(size, s, text_end, wrap_width - (x - pos.x));
	...
	inline void ImDrawList_AddAnsiText(ImDrawList* drawList,
			ImFont* font,
			float font_size,
			const ImVec2& pos,
			ImU32 col,
			...
			const ImVec4* cpu_fine_clip_rect = NULL)
	{
		...
		// IM_ASSERT(font->ContainerAtlas->TexData->TexID == drawList->_TextureStack.back());  // Use high-level ImGui::PushFont()
		// or low-level
	...
	inline void RenderAnsiText(ImVec2 pos, const char* text, const char* text_end, bool hide_text_after_hash)
	{
		...
		if (text != text_display_end) {
			ImGui::PushFont(g.Font, g.FontSize);
	...
	inline void RenderAnsiTextWrapped(ImVec2 pos, const char* text, const char* text_end, float wrap_width)
	{
		ImGuiContext& g = *GImGui;
		ImGuiWindow* window = g.CurrentWindow;
	...
	inline void TextAnsiUnformatted(const char* text, const char* text_end)
	{
	...
	inline void TextAnsiV(const char* fmt, va_list args)
	{
		ImGuiWindow* window = GetCurrentWindow();
		if (window->SkipItems)
			return;
	...
	inline void TextAnsiColoredV(const ImVec4& col, const char* fmt, va_list args)
	{
	...
	inline void TextAnsiColored(const ImVec4& col, const char* fmt, ...)
	{
		va_list args;
		va_start(args, fmt);
